...
impl Universe {
    ...
    pub fn tick(&mut self) {
        ...
        self.add_particles_thrust_forces();
    ...
    //
    // Return particles data used for drawing.
    // Fo each particle, returns :
    //  position.x ;
    //  position.y ;
    //  radius ;
    //  thrust_force or 0.0
    ...
    pub fn get_particles_data_to_draw(&self) -> Vec<f64> {
        ...
        for particle in self.particles.iter() {
            if particle.is_enabled() {
                ...
                if particle.thrust_activated() {
                    data.push(particle.thrust_force());
                } else {
                    data.push(0.0);
                }
            } else {
    ...
    //
    // Activate thrust for the given particles
    //
    pub fn activate_thrust_for_particles(&mut self, particle_indexes: Vec<usize>) {
        for particle_index in particle_indexes {
            if particle_index < self.particles.len() {
                self.particles[particle_index].activate_thrust();
            } else {
                console_warning!("Cannot activate thrust : particle #{} doesn't exist.", particle_index);
            }
        }
    }

    //
    // Deactivate thrust for the given particles
    //
    pub fn deactivate_thrust_for_particles(&mut self, particle_indexes: Vec<usize>) {
        for particle_index in particle_indexes {
            if particle_index < self.particles.len() {
                self.particles[particle_index].deactivate_thrust();
            } else {
                console_warning!("Cannot deactivate thrust : particle #{} doesn't exist.", particle_index);
            }
        }
    }
...
impl Universe {
    ...
    //
    // Add thrust forces generated by particles to other particles
    //
    fn add_particles_thrust_forces(&mut self) {
        let particles = self.particles.clone();
        for (i, p1) in particles.iter().enumerate() {
            for (j, p2) in particles.iter().enumerate() {
                if i < j && (p1.thrust_activated() || p2.thrust_activated()) {
                    let p1_thrust_force = if p1.thrust_activated() {
                        p1.thrust_force()
                    } else {
                        0.0
                    };
                    let p2_thrust_force = if p2.thrust_activated() {
                        p2.thrust_force()
                    } else {
                        0.0
                    };
                    let thrust_force_magnitude = p1_thrust_force + p2_thrust_force;
                    let p1_to_p2 = Vector::new(
                        & p1.get_coordinates_as_point(),
                        & p2.get_coordinates_as_point()
                    );
                    let distance = p1_to_p2.length();
                    let thrust_force = p1_to_p2
                        .normalized()
                        .multiplied(
                            thrust_force_magnitude / (distance * distance)
                        );
                    match (p1.is_fixed(), p2.is_fixed()) {
                        (false, false) => {
                            self.particles[i].add_force(thrust_force.multiplied(-0.5));
                            self.particles[j].add_force(thrust_force.multiplied(0.5));
                        },
                        (false, true) => {
                            self.particles[i].add_force(thrust_force.multiplied(-1.0));
                        },
                        (true, false) => {
                            self.particles[j].add_force(thrust_force.multiplied(1.0));
                        },
                        (true, true) => {
                            // Do nothing
                        }
