...
pub fn compute_collision_responses(
    ...
    unsafe {
        let particles_pointer = particles as *mut [Particle];
        let particles_internal_pointer = particles_internal as *mut [ParticleInternal];
        for p1 in particles.iter_mut() {
            if p1.live == 0 {
                continue;
            }
            let p1_ = &mut (*particles_pointer)[p1.idx];
            for pid2 in neighbours(&p1.p, grid).iter().flat_map(|x| *x) {
                let p2 = &mut (*particles_pointer)[*pid2];
                if p2.live == 0 {
                ...
                if p1.idx < p2.idx {
                    ...
                    if wa.d_sqrd < diameter_sqrd {
                        ...
                        let pi2 = &mut (*particles_internal_pointer)[p2.idx];
...
pub fn compute_collision_responses_2(
    ...
    unsafe {
        ...
        for p1 in particles.iter_mut() {
            ...
            let gid = grid_id_position(p1.p, grid.side);
            let uu = &grid.gids[gid];
            for pid2 in [
                &grid.pidxs[uu[0]],
                &grid.pidxs[uu[1]],
