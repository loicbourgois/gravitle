...
fn main([[builtin(global_invocation_id)]] gid : vec3<u32>) {
  ...
  if (part_id <= u32(data.count) ) {
    ...
    for (var i = -r ; i < r ; i=i+resolution ) {
      for (var j = -r2 ; j < r2 ; j=j+resolution ) {
        ...
        let pix_id = u32(xx) + u32(yy) * ${x.image_width}u;

        let oi = 1.0 - sqrt(i*i+j*j) / (d * zoom*0.5);

        if (i*i+j*j < r*r2) {
          output.pix[pix_id].r = 100u + u32(255.0*oi);
          output.pix[pix_id].g = u32(255.0*oi);
