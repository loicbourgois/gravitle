...
const render = () => {
  if (! (server_data instanceof ArrayBuffer) ) {
    window.requestAnimationFrame(render);
    return
  }
  let ii = 0;
  const view = new DataView(server_data);
  const server_timestamp = view.getBigInt64(ii) ; ii+=8
  const client_timestamp = BigInt( (new Date()).getTime() );
  if (client_timestamp > server_timestamp + BigInt(100) ) {
    window.requestAnimationFrame(render);
    return
  }
  ...
  texts.innerHTML = `
    ...
    <p>clients: ${clients}</p>
    <p>bytes: ${server_data.byteLength}</p>
