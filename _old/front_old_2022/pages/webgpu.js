...
const counters = {}
for (var k of ['put_image', 'map', 'await_map']) {
  counters[k] = {
...
async function render_webgpu(server, x, ctx) {
  ...
  start_counter('await_map');
  await read_map;
  await write_map;
  await write_data_map;
  stop_counter('await_map');
  ...
  // for (var k of counters) {
  //   console.log(k)
  // }
  for (let k in counters) {
    if (len(counters[k].values) > 0) {
      while (len(counters[k].values) > counters[k].max) {
      ...
      v /= len(counters[k].values)
      document.getElementById(`p_counter_${k}`).innerHTML = `${k}: ${v.toFixed(2)}ms`
    }
  }

  document.getElementById("p_counter_global").innerHTML =     `Compute:     ${ server.get_counter_value(0).toFixed(2) }ms`
