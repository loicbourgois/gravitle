...
def run(epochs=None, num_samples=None):
    _context = ray.init()
    config = {
        "lr": tune.uniform(0.5, 1),
        "patience": tune.uniform(1, 10),
        "reduce_factor": tune.uniform(0.1, 1),
        "l1": tune.randint(4, 20),
        "l2": tune.randint(4, 20),
        "l3": tune.randint(4, 20),
        'epochs': epochs,
    }
    tuner = tune.Tuner(
        Trainable,
        ...
        run_config=air.RunConfig(progress_reporter=CustomReporter(num_samples)),
        param_space=config,
    )
